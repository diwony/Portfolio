(function () {
  ...
  var PROJECTS = [
    ...
    },
    {
      ...
      overview: '냉장고에 있는 재료를 입력하면 만들 수 있는 유튜브 요리 영상을 찾아주고, 조리 스텝마다 붙은 타임스탬프를 누르면 영상의 그 장면으로 바로 이동하는 요리 도우미 웹·모바일 앱. 재료가 없으면 기분·상황만 골라도 되고(냉장고·밀키트·장보기·디저트 4가지 시작 모드), 검색 없이 훑어보다 발견하는 홈과 자취생·1인가구·직장인 등 페르소나 맞춤 추천을 제공. 1인 개인 프로젝트로 기획·디자인·프론트엔드는 물론 자막을 스텝·타임스탬프로 바꾸는 Claude API 빌드 타임 파이프라인까지 전 과정을 담당. 코드 약 7.3k줄, 큐레이션 레시피 25개, 유튜브 영상 풀 11,200+.',
      problems: [
        '레시피 영상에서 원하는 조리 장면을 찾기 번거로운 문제 → 스텝마다 타임스탬프 seek, 스크롤 시 우상단 미니 플레이어(PiP)로 고정. iframe 노드를 옮기거나 다시 만들지 않고 CSS 클래스만 바꿔 재생이 끊기지 않게 구현',
        '"가진 재료로 뭘 만들지 모르겠다" + 재료를 하나씩 입력하는 마찰 → 재료·기분·상황을 칩 또는 자유 문장으로 받아 결과가 실제로 좁혀지는 매칭·랭킹 로직. vibe는 하드 필터가 아닌 정렬 가중치라 키워드를 잘못 잡아도 결과가 사라지지 않음',
        '조회수로 정렬하면 한 채널이 목록 상단을 독식하는 문제 → 같은 채널이 반복될수록 커지는 감점을 주고 그리디하게 재정렬, 점수 차가 큰 상위권은 그대로 두고 비슷한 후보들 사이에서만 채널을 번갈아 노출',
        '정적 배포(GitHub Pages)라 클라이언트에 YouTube API 키를 둘 수 없는 문제 → 키를 시크릿으로 숨긴 Cloudflare Workers 프록시(KV 24h 캐시·IP당 rate limit) 경유, 모든 실패를 빈 배열로 흡수해 11,000+ 정적 영상 풀로 조용히 폴백',
        '웹(React)과 모바일(React Native)의 코드 중복 문제 → 매칭·랭킹·데이터 로직을 의존성 0인 @foodplay/core 패키지로 분리해 UI만 각자 만들고 나머지는 100% 공유, 영상 제어도 seekTo·pause 같은 계약으로 통일'
      ],
